...
use druid::{kurbo::Line, piet::PietText, widget::Label};
use druid::text::{
...
use crate::env::key::{PRIMARY, PRIMARY_VARIANT};
...
impl<T: TextStorage + EditableText> Widget<T> for TextBox<T> {
    fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut T, _env: &Env) {
        ...
        match event {
            ...
            Event::Command(ref cmd) if ctx.is_focused() && cmd.is(druid::commands::CUT) => {
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, data: &T, env: &Env) {
        // TODO: Set a min size in order to render properly, and add the animation when on_focuse
        let size = ctx.size();
        let background_color = env.get(theme::WINDOW_BACKGROUND_COLOR);
        let selection_color = env.get(theme::SELECTION_COLOR);
        let cursor_color = env.get(theme::CURSOR_COLOR);
        let border_width = env.get(theme::TEXTBOX_BORDER_WIDTH);
        let text_insets = env.get(theme::TEXTBOX_INSETS);
        if background_color == Color::rgb8(0x29, 0x29, 0x29) {
        ...
        // The Line color under text
        let before_focus = Color::GRAY;
        let line_color = env.get(PRIMARY_VARIANT);
        let line = Line::new((0.,size.height),(size.width - border_width,size.height));
        ...
        let clip_rect = Size::new(size.width - border_width, size.height)
            ...
            // Paint line
            if !is_focused {
                ctx.stroke(line, &before_focus, border_width);
            } else {
                ctx.stroke(line, &line_color, border_width);
